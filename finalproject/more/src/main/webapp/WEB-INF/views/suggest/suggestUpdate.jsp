	...
	<div class="revMainContainer">
		<div class="revMainTextBox">
			<div class="revMainText">
				<h1>
					제안서 수정<br>
					<span style="font-size:25px; font-weight:normal; border-top: 1px solid white; padding: 5px;">제안서를 수정해보세요</span>
				</h1>	
			</div>
		</div>
	</div>
	<form name="suggestUpdateForm" action="suggestUpdate.do" method="post">
		<div class="container">
			<!-- 레슨시작일/종료일 -->
			...
			<div class="periodContainer">
				...
				<div class="items">
					<div class="item">
						<h5>레슨 시작일</h5>
						<input type="date" name="start_date" class="dateItem" value="${dto.start_date}">
					</div>
					<div class="item">
						<h5>레슨 종료일</h5>
						<input type="date" name="end_date" class="dateItem" value="${dto.end_date}">
					</div>
				</div>
			</div>
					...
					<div class="item">
						...
						<select name="start_time">
							...
							<c:forEach var="i" begin="01" end="24">
								<option value="${i}" <c:if test="${dto.start_time eq i}">selected</c:if>>${i}:00</option>
							</c:forEach>
						</select>
					</div>
					<div class="item">
						...
						<select name="end_time">
							...
							<c:forEach var="i" begin="01" end="24">
								<option value="${i}" <c:if test="${dto.end_time eq i}">selected</c:if>>${i}:00</option>
							</c:forEach>
						</select>
					</div>
				</div>
			...
			<div class="costContainer">
				...
				<div class="items">
					<div class="item">
						<h5>레슨 비용</h5>
						<select name="cost">
							<option value="">레슨 비용</option>
							<c:forEach var="i" begin="5" end="50" step="5">
								<option value="${i}" <c:if test="${dto.cost eq i}">selected</c:if>>${i}만원</option>
							</c:forEach>
						</select>
					</div>
				</div>
			...
			<div class="processContainer">
				...
				<ul>
					...
					<input type="checkbox" name="process" value="의뢰인이 있는 곳으로 이동 가능합니다" id="id_travel_option_0"
					<c:if test="${dto.process eq '의뢰인이 있는 곳으로 이동 가능합니다'}">checked</c:if>>의뢰인이 있는 곳으로 이동 가능합니다</label></li>
					<li><label for="id_travel_option_1">
					<input type="checkbox" name="process" value="제가 있는 곳으로 의뢰인이 오는게 좋겠습니다" id="id_travel_option_1"
					<c:if test="${dto.process eq '제가 있는 곳으로 의뢰인이 오는게 좋겠습니다'}">checked</c:if>>제가 있는 곳으로 의뢰인이 오는게 좋겠습니다</label></li>
			...
			<div class="ls_locationContainer">
				<h4>어디서 진행하면 좋을까요?</h4>
					...
					<input type="text" name="ls_location" id="sample3_detailAddress" value="${dto.ls_location}" placeholder="주소를 찾아주세요" readonly>
			...
			<div class="introContainer">
				<h4>학생분들에게 정보를 조금 더 알려주세요.</h4>
				<textarea name="s_intro" cols="40" rows="23" autocomplete="off" class="input-lg form-control"
					placeholder="관련 경험이나 실력, 구체적인 목적이나 집중하고 싶은 분야가 있다면 알려주세요. 이 외에도 수업 진행시 선생님이 갖추어야 할 조건이 있다면 반드시 언급해주세요."
					maxlength="1000" required="" id="id_field_25">${dto.s_intro}</textarea>
			</div>
			<div class="updateBt">
