...
public class RefundDonePageModule {

   public static String makePageRd(String pageName,int totalCnt,int listSize,int pageSize,int cprd) {
         
         //페이지의 모듈화///////// 원하는 곳에서 사용이 가능하다~~~~~~~~~~~~~~~~~~~~~~~~~~~
         
         int totalPage=(totalCnt/listSize)+1; //총페이지수
         if(totalCnt%listSize==0)totalPage--;    //딱나눠떨어질 경우 페이지만들기위해 -1
         int userGroup=cprd/pageSize; //userGroup=사용자의 현재그룹
         if(cprd%pageSize==0)userGroup--;
